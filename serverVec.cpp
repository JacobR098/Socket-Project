...
int main(int argc, char* argv[]) {
	...
	for (;;) {
		...
		vector<int> vec;
		int numExpected, recvInt;
		int* arr;
		
		//Block until receive message from a client
		if ((recvMsgSize = recvfrom(sock, &numExpected, sizeof(numExpected), 0, (struct sockaddr*) &echoClntAddr, &cliAddrLen)) < 0)
			DieWithError("recvfrom() failed\n");

		arr = new int[numExpected];
		for(int i = 0; i < numExpected; i++){
		  recvfrom(sock, &arr[i], sizeof(int), 0, (struct sockaddr*) &echoClntAddr, &cliAddrLen);
		  //vec.push_back(recvInt);
		  }
